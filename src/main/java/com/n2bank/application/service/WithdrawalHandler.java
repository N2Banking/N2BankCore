...
/** Applies withdrawal rules inside the shared operation transaction. */
public final class WithdrawalHandler {
  private final OperationExecutor executor;
  private final FeeService fees;
  private final Function<Currency, UUID> revenueAccounts;
  private final Function<Currency, UUID> cashAccounts;

  public WithdrawalHandler(OperationExecutor executor, FeeService fees, Function<Currency, UUID> revenueAccounts, Function<Currency, UUID> cashAccounts) {
  ...
  public OperationResult handle(WithdrawalCommand command) {
    ...
    return executor.execute(command, accounts -> {
      ...
      FeeQuote fee = fees.calculate(FeeType.WITHDRAWAL, new FeeContext(command.amount(), source));
      lines.add(new Posting(source.accountId(), command.amount().add(fee.amount()), Direction.DEBIT));
      lines.add(new Posting(cash.accountId(), command.amount(), Direction.CREDIT));
      HandlerSupport.addFee(lines, fee.amount(), accounts, revenueAccounts);
      return HandlerSupport.entry(command, "Withdrawal with " + fee.description(), lines);
    });
  }
}
